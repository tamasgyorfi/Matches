...
import hu.bets.matches.model.{MatchResult, ScheduledMatch, Team}
...
class MatchInfoGatewayTest extends JUnitSuite with MockitoSugar {
  ...
  def shouldParseResults(): Unit = {
    ...
    List(MatchResult(new ScheduledMatch("sr:match:9792041", LocalDateTime.parse("2017-06-03T11:00"), "Super Lig", new Team("Kayserispor", "KAY", "Turkey"), new Team("Basaksehir FK","BAS","Turkey")), 0, 1))
  }

  @Test
  def shouldReturnOneDaysWorthOfResults(): Unit = {
    sut.getMatchResults === List(MatchResult(new ScheduledMatch("sr:match:9792041", LocalDateTime.parse("2017-06-03T11:00"), "Super Lig", new Team("Kayserispor", "KAY", "Turkey"), new Team("Basaksehir FK","BAS","Turkey")), 0, 1))
  }
}
